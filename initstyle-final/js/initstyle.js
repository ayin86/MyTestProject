...
function IntObject2(){
    ...
    var rab=$(".radioBtn");
    var ckb=$(".checkBtn");
    var ibtn=$("input:button");
    var btnNC=$(".buttonNC")
    var all=$(btn).add(rab).add(ckb).add(btnNC);//.add(ibtn);
    ...
    $(sn).each(function(i,sn2){
        ...
        $(all).each(function(j,ano){
           ...
           if($(ano).is("ul")){
               ...
               $(chi).addClass("navLi navLi"+step).first().addClass("navLiFrist").end().last().addClass("navLiLast");
...
function IntImgSwitch(){
    ...
    $(imgs).each(function(i,imgs2){
        ...
        var objPosi=function(trans,text,img2,num2){
            $(trans).fadeIn(300);
            $(text).fadeIn(300);
            $(trans).css({width:$(img2).width()-2,height:$(text).outerHeight(true)-2,"z-index":9});
            $(num2).parent().stop().animate({bottom:$(trans).height()+10},200);
        }
        ...
        $(num).each(function(inum,num2){$(num2)
            ...
            .bind("click",function(){
                ...
                $(img).each(function(ini,img2){
                    if(ini==inum){
                        $(img2).fadeIn(500);
                        var trans=$(img2).children(".trans");
                        var text=$(img2).children(".desc");
        ...
        setTimeout(function(){$(img).eq(0).fadeIn(300,function(){$(img).not(":eq(0)").show(0);});},1000);
        $(img).each(function(j,img2){
            var imgf=$(img2).children("img");
            ...
            var text=$(img2).children("div");
            if($(text).length>0){
                var h5=$(text).children("h5");
                $(text).addClass("desc");
                $(h5).detach();
                $(text).wrapInner("<span\/>").prepend(h5).hide();
                var det=$(text).children("span");
                $(det).text($.trim($(det).text()));
                EventCutStr2(h5,$(img2));
                EventCutStr2(det,$(img2),"","x2 +4");
                $(img2).prepend("<div class='trans'/>");
                var trans=$(img2).children(".trans");
                $(trans).fadeTo(0,0.5).hide();
            }
        });
        
        var imgIntv;
        var interval=$(imgs2).data("interval");
        if(interval==null){interval=5000;}
        function intvImgS(){
            imgIntv=window.setInterval(function(){
                var actnum=$(numWrap).children(".active");
                if($(actnum).next().length!=0){$(actnum).removeClass("active");$(actnum).next().addClass("active");}
                else{$(actnum).removeClass("active");$(num).eq(0).addClass("active");}
                $(img).each(function(j,img2){
                    var trans=$(img2).children(".trans");
                    var text=$(img2).children(".desc");
                    $(num).each(function(jnum,num2){
        ...
        intvImgS();
        $(imgs2).parent(".imgSwitchWrap").hover(
